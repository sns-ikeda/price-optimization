    ...
    def run(self) -> None:
        ...
        for target_col in self.target_cols:
            ...
            plot(
                ...
                target_item=predictor.item,
                suffix="train",
            )

            # 学習データを用いたときの平均二乗誤差を出力
            rmse_train = round(np.sqrt(mean_squared_error(y_train, y_pred_train)), 2)
            self.result["rmse"]["train"][predictor.item] = rmse_train
            logger.info(f"RMSE for train data [{predictor.item}]: {rmse_train}")
            ...
            if self.test_df is not None:
                ...
                self.result["rmse"]["test"][predictor.item] = rmse_test
                logger.info(f"RMSE for test data [{predictor.item}]: {rmse_test}")
